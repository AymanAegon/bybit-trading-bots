...
SYMBOLS = ['ADAUSDT', 'BTCUSDT', 'LINKUSDT', 'ARBUSDT']
TIMEFRAMES = ['1', '5']

if len(sys.argv) != 4:
	print("Error: invalid arguments!!\nYou need SYMBOL QUANTITY TIMEFRAME")
	quit()
elif sys.argv[1] not in SYMBOLS:
	print("Error: Symbol not supported!!\nSupported Symbols:")
	print(SYMBOLS)
	quit()
elif sys.argv[3] not in TIMEFRAMES:
	print("Error: Timeframe not supported!!\nSupported Timeframes:")
	print(TIMEFRAMES)
	quit()
...
# Strategy parameters
symbol = sys.argv[1]
qty = float(sys.argv[2])  # Adjust the trade quantity as needed
timeframe = sys.argv[3]  # Use "1" for 1-minute; change to "5" for 5-minute timeframe
...
def place_order2(side, qty):
	print("Order placed: ", side, qty)
...
def close_position2(side, qty):
	print("Position closed: ", side, qty)
        ...
        if df is not None:
            ...
            if open_pos:
                ...
                if signal == 'long':
                    ...
                    place_order2("Buy", qty)
                elif signal == 'short':
                    logging.info("Placing new short order.")
                    place_order2("Sell", qty)
                else:
